...
def get_current_scores(db: Session = Depends(get_db), current_user: AppUser = Depends(get_current_user)):
    ...
    A function with no scan history yet defaults to Tier 1 rather than being
    omitted, so the radar chart always has a full six-function shape to draw.
    """
    ...
    for fn in functions:
        ...
        results.append(MaturityScoreOut(
            ...
            calculated_at=latest.calculated_at if latest else datetime.utcnow(),
        ))
    return results
