...
| GET | `/api/v1/drift/forecast` | Drift forecast (trend + projected tier) per NIST function |
...
| GET | `/api/v1/audit-log/verify` | Verify the audit trail's hash chain is unbroken (admin only) |

---

## 9. Compliance drift forecasting

The scoring engine and risk quantifier both answer "where do things stand right
now." The drift forecaster (`app/services/drift_forecaster.py`) instead asks
whether each NIST CSF 2.0 function is trending toward a *lower* tier, so a
CISO can act before a control actually fails instead of finding out after.

It uses whichever signal there's enough data for:

- **Once an org has at least 3 scans on record**, it fits a least-squares line
  through that function's historical `MaturityScore` tiers over time and
  projects it out 30 and 90 days. The fit's R² doubles as a confidence score —
  a noisy history is reported with lower confidence than a clean trend.
- **Before that**, there isn't enough score history to regress on, so it falls
  back to comparing non-compliant finding counts per subcategory over the last
  14 days against the 14 days before that. A subcategory accumulating
  non-compliant findings faster than before is flagged as an early warning
  even with no tier history yet.

The two methods are never blended into one number — the API reports which
method produced a given forecast (`regression`, `velocity`, or
`insufficient_data`) so the dashboard doesn't present a two-point guess with
the same confidence as a real trend line. See `GET /api/v1/drift/forecast`
and the **Drift Outlook** page in the dashboard.

This is a genuine gap in most GRC/CSPM tooling, which is almost entirely
point-in-time: they'll tell you a control is failing, rarely that it's about
to. Worth calling out specifically if this project is being evaluated against
similar off-the-shelf tools.

---

## 10. Tamper-evident audit log

The audit trail (`app/services/audit_logger.py`) was previously just an
append-only table by convention — nothing actually stopped someone with
direct database access from editing or deleting a row, and nothing would
show it had happened. Every entry is now hash-chained: each row's
`entry_hash` is a SHA-256 digest over its own content *plus* the previous
row's `entry_hash`, the same construction used by tamper-evident logs and
blockchains generally. Editing or deleting any historical row breaks the
hash of every row computed after it.

`GET /api/v1/audit-log/verify` (admin only) walks the full chain and
reports the first row where the recorded hash no longer matches what it
should be, meaning that row — or an earlier one — was altered outside the
application.

Honest limits, documented rather than glossed over: this doesn't stop
someone with DB write access from recomputing every downstream hash to
cover their tracks, and it isn't a substitute for shipping the log
somewhere outside this database entirely. What it catches is a one-off
edit made without also rewriting the whole chain — which is the realistic
case (a support engineer "fixing" a row, a bug, a bad migration) far more
often than a sophisticated cover-up. `seq` assignment also assumes
single-writer-at-a-time; see the docstring in `audit_logger.py` for what
that would take to harden further.
...
original document. Compliance drift forecasting (Section 9) and the tamper-evident
audit log (Section 10) extend beyond the original blueprint scope.
